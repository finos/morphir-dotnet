...
let tests =
    describe
        ...
        [ describe
              "contains Tests"
              [ let digit = Maybe.withDefault Regex.never <| Regex.fromString "[0-9]"
          ...
          describe
              "Splitting Tests"
              [ let comma = Maybe.withDefault Regex.never <| Regex.fromString " *, *"

                describe
                    "split Tests"
                    [ testCase @"split comma ""tom,99,90,85"""
                      <| fun _ -> Expect.equal ([ "tom"; "99"; "90"; "85" ]) (Regex.split comma "tom,99,90,85")
                      testCase @"split comma ""tom, 99, 90, 85"""
                      <| fun _ -> Expect.equal ([ "tom"; "99"; "90"; "85" ]) (Regex.split comma "tom, 99, 90, 85")
                      testCase @"split comma ""tom , 99, 90, 85"""
                      <| fun _ -> Expect.equal ([ "tom"; "99"; "90"; "85" ]) (Regex.split comma "tom , 99, 90, 85") ]
                ...
                describe
                    ...
                    [ testCase @"splitAtMost comma ""tom,99,90,85"""
                      <| fun _ -> Expect.equal ([ "tom"; "99"; "90,85" ]) (Regex.splitAtMost 2 comma "tom,99,90,85") ] ] ]
