...
let tests =
    let combiningTests =
        describe "Combining Strings" [
            testCase "cons"
            <| fun _ ->
                Expect.equal "The truth is out there" (String.cons 'T' "he truth is out there")
            testCase "uncons non-empty"
            <| fun _ -> Expect.equal (Just('a', "bc")) (String.uncons "abc")
            testCase "uncons empty"
            ...
            <| fun _ -> Expect.equal 0 (String.length null)
            ...
            <| fun _ ->
                Expect.equal
                    ...
                    (String.replace "Replac" "Mov" "Replacing the replacement")
            testCase "append"
            ...
            testCase "left 0"
            <| fun _ -> Expect.equal "" (String.left 0 "Number")
            testCase "left"
            <| fun _ -> Expect.equal "Mu" (String.left 2 "Mulder")

            testCase "right 0"
            <| fun _ -> Expect.equal "" (String.right 0 "Right")
            testCase "right"
            <| fun _ -> Expect.equal "ly" (String.right 2 "Scully")

            testCase "dropLeft 0"
            <| fun _ -> Expect.equal "Something" (String.dropLeft 0 "Something")
            testCase "dropLeft"
            <| fun _ -> Expect.equal "one" (String.dropLeft 2 "Alone")

            testCase "dropRight 0"
            <| fun _ -> Expect.equal "Left" (String.dropRight 0 "Left")
            testCase "dropRight"
            <| fun _ -> Expect.equal "Dr" (String.dropRight 2 "Drop")
            ...
            testCase "toInt2"
            <| fun _ -> Expect.equal (Some -42) (String.toInt "-42")
            testCase "toInt3"
            <| fun _ -> Expect.equal None (String.toInt "0a")

            testCase "fromInt1"
            <| fun _ -> Expect.equal "123" (String.fromInt 123)
            testCase "fromInt2"
            <| fun _ -> Expect.equal "-42" (String.fromInt -42)

            testCase "toFloat1"
            <| fun _ -> Expect.equal (Some 123.0) (String.toFloat "123")
            testCase "toFloat2"
            <| fun _ -> Expect.equal (Some -42.0) (String.toFloat "-42")
            testCase "toFloat3"
            <| fun _ -> Expect.equal (Some 3.1) (String.toFloat "3.1")
            testCase "toFloat4"
            <| fun _ -> Expect.equal None (String.toFloat "31a")

            testCase "fromFloat1"
            <| fun _ -> Expect.equal "123" (String.fromFloat 123.0)
            testCase "fromFloat2"
            <| fun _ -> Expect.equal "-42" (String.fromFloat -42.0)
            testCase "fromFloat3"
            <| fun _ -> Expect.equal "3.1" (String.fromFloat 3.1)
            ...
            testCase "toLower"
            <| fun _ -> Expect.equal "skinner" (String.toLower "SKINNER")

            testCase "pad1"
            <| fun _ -> Expect.equal "  1  " (String.pad 5 ' ' "1")
            testCase "pad2"
            <| fun _ -> Expect.equal "  11 " (String.pad 5 ' ' "11")
            testCase "pad3"
            <| fun _ -> Expect.equal " 121 " (String.pad 5 ' ' "121")

            testCase "padLeft1"
            <| fun _ -> Expect.equal "....1" (String.padLeft 5 '.' "1")
            testCase "padLeft2"
            <| fun _ -> Expect.equal "...11" (String.padLeft 5 '.' "11")
            testCase "padLeft3"
            ...
            testCase "trim"
            <| fun _ -> Expect.equal "hats" (String.trim "  hats  \n")
            testCase "trimLeft"
