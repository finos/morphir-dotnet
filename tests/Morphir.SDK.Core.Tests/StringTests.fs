...
let tests =
    let combiningTests =
        describe "Combining Strings" [
            ...
            testCase "reverse" <| fun _ -> Expect.equal "stressed" (String.reverse "desserts")
            testCase "repeat" <| fun _ -> Expect.equal "EchoEchoEcho" (String.repeat 3 "Echo")
            testCase "replace" <| fun _ -> Expect.equal "Moving the replacement" (String.replace "Replac" "Mov" "Replacing the replacement")
            testCase "append" <| fun _ -> Expect.equal "HelloWorld" (String.append "Hello" "World")
            testCase "split" <| fun _ -> Expect.equal ["earth"; "quake"] (String.split " " "earth quake")
            testCase "concat" <| fun _ -> Expect.equal "Coming together" (String.concat ["Coming"; " "; "together"])
            testCase "words" <| fun _ -> Expect.equal ["Breaks"; "up"; "all"; "the"; "words"] (String.words "Breaks up all the words")
            testCase "lines" <| fun _ -> Expect.equal ["New"; "Line"] (String.lines "New\nLine")

            testCase "slice1" <| fun _ -> Expect.equal "on" (String.slice 7 9 "snakes on a plane!")
            testCase "slice2" <| fun _ -> Expect.equal "snakes" (String.slice 0 6 "snakes on a plane!")
            testCase "slice3" <| fun _ -> Expect.equal "snakes on a" (String.slice 0 -7 "snakes on a plane!")
            testCase "slice4" <| fun _ -> Expect.equal "plane" (String.slice -6 -1 "snakes on a plane!")
            ...
            testCase "dropLeft 0" <| fun _ -> Expect.equal "Something" (String.dropLeft 0 "Something")
            testCase "dropLeft" <| fun _ -> Expect.equal "one" (String.dropLeft 2 "Alone")

            testCase "dropRight 0" <| fun _ -> Expect.equal "Left" (String.dropRight 0 "Left")
            testCase "dropRight" <| fun _ -> Expect.equal "Dr" (String.dropRight 2 "Drop")

            testCase "contains1" <| fun _ -> Expect.equal true (String.contains "the" "theory")
            testCase "contains2" <| fun _ -> Expect.equal false (String.contains "hat" "theory")
            testCase "contains3" <| fun _ -> Expect.equal false (String.contains "THE" "theory")

            testCase "startsWith1" <| fun _ -> Expect.equal true (String.startsWith "the" "theory")
            testCase "startsWith2" <| fun _ -> Expect.equal false (String.startsWith "ory" "theory")

            testCase "endsWith1" <| fun _ -> Expect.equal false (String.endsWith "the" "theory")
            testCase "endsWith2" <| fun _ -> Expect.equal true (String.endsWith "ory" "theory")

            testCase "indexes i" <| fun _ -> Expect.equal [1;4;7;10] (String.indexes "i" "Mississippi")
            testCase "indexes ss" <| fun _ -> Expect.equal [2;5] (String.indexes "ss" "Mississippi")
            testCase "indexes none" <| fun _ -> Expect.equal [] (String.indexes "needle" "haystack")
            testCase "indices i" <| fun _ -> Expect.equal [1;4;7;10] (String.indices "i" "Mississippi")
            ...
            testCase "toList" <| fun _ -> Expect.equal ['a';'b';'c'] (String.toList "abc")
            testCase "fromList" <| fun _ -> Expect.equal "abc" (String.fromList ['a';'b';'c'])
            ...
            testCase "padLeft3" <| fun _ -> Expect.equal "..121" (String.padLeft 5 '.' "121")

            testCase "padRight1" <| fun _ -> Expect.equal "1...." (String.padRight  5 '.' "1")
            testCase "padRight2" <| fun _ -> Expect.equal "11..." (String.padRight  5 '.' "11")
            testCase "padRight3" <| fun _ -> Expect.equal "121.." (String.padRight  5 '.' "121")

            testCase "trim" <| fun _ -> Expect.equal "hats" (String.trim  "  hats  \n")
            testCase "trimLeft" <| fun _ -> Expect.equal "hats  \n" (String.trimLeft  "  hats  \n")
            testCase "trimRight" <| fun _ -> Expect.equal "  hats" (String.trimRight  "  hats  \n")

            testCase "map" <| fun _ -> Expect.equal "a.b.c" (String.map (fun c -> if c.Equals '/' then '.' else c) "a/b/c")
            testCase "filter" <| fun _ -> Expect.equal "22" (String.filter System.Char.IsDigit "R2-D2")
