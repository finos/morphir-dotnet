...
module Helpers =
    ...
    let typeVar (name: string) : Type.Type<unit> =
        Type.Variable((), Name.fromString name)

    /// <summary>
    /// Creates a type variable from a Name.
    /// </summary>
    let typeVarName (name: Name) : Type.Type<unit> =
        Type.Variable((), name)
    ...
    let typeRef (fqName: FQName) : Type.Type<unit> =
    ...
    let typeRefWith (fqName: FQName) (typeArgs: Type.Type<unit> list) : Type.Type<unit> =
    ...
    let typeTuple (elements: Type.Type<unit> list) : Type.Type<unit> =
        Type.Tuple((), elements)
    ...
    let typeRecord (fields: Type.Field<unit> list) : Type.Type<unit> =
        Type.Record((), fields)
    ...
    let typeFunc (argType: Type.Type<unit>) (returnType: Type.Type<unit>) : Type.Type<unit> =
    ...
    /// <param name="returnType">The return type</param>
    let typeFuncList (paramTypes: Type.Type<unit> list) (returnType: Type.Type<unit>) : Type.Type<unit> =
        List.foldBack (fun param acc -> Type.Function((), param, acc)) paramTypes returnType

    /// <summary>
    /// Creates a unit type.
    /// </summary>
    let typeUnit () : Type.Type<unit> = Type.Unit()
    ...
    let typeField (name: string) (typ: Type.Type<unit>) : Type.Field<unit> =
        Type.field (Name.fromString name) typ

    /// <summary>
    /// Creates a type field from a Name.
    /// </summary>
    let typeFieldName (name: Name) (typ: Type.Type<unit>) : Type.Field<unit> =
        Type.field name typ
    ...
    /// </summary>
    let litBool (value: bool) : Literal.Literal = Literal.BoolLiteral value

    /// <summary>
    /// Creates a character literal.
    /// Shorthand for Literal.CharLiteral.
    /// </summary>
    let litChar (value: char) : Literal.Literal = Literal.CharLiteral value

    /// <summary>
    /// Creates a string literal.
    /// Shorthand for Literal.StringLiteral.
    /// </summary>
    let litString (value: string) : Literal.Literal = Literal.StringLiteral value

    /// <summary>
    /// Creates an integer literal.
    /// Shorthand for Literal.WholeNumberLiteral.
    /// </summary>
    let litInt (value: int64) : Literal.Literal = Literal.WholeNumberLiteral value

    /// <summary>
    /// Creates a float literal.
    /// Shorthand for Literal.FloatLiteral.
    /// </summary>
    let litFloat (value: float) : Literal.Literal = Literal.FloatLiteral value

    /// <summary>
    /// Creates a decimal literal.
    /// Shorthand for Literal.DecimalLiteral.
    /// </summary>
    let litDecimal (value: decimal) : Literal.Literal = Literal.DecimalLiteral value
    ...
    /// </summary>
    let patWildcard () : Pattern.Pattern<unit> = Pattern.WildcardPattern()
    ...
    let patAs (pattern: Pattern.Pattern<unit>) (name: string) : Pattern.Pattern<unit> =
        Pattern.AsPattern((), pattern, Name.fromString name)

    /// <summary>
    /// Creates an as-pattern with Name.
    /// </summary>
    let patAsName (pattern: Pattern.Pattern<unit>) (name: Name) : Pattern.Pattern<unit> =
    ...
    let patTuple (patterns: Pattern.Pattern<unit> list) : Pattern.Pattern<unit> =
    ...
    let patCtor (fqName: FQName) (patterns: Pattern.Pattern<unit> list) : Pattern.Pattern<unit> =
    ...
    /// </summary>
    let patEmptyList () : Pattern.Pattern<unit> = Pattern.EmptyListPattern()
    ...
    let patHeadTail (headPattern: Pattern.Pattern<unit>) (tailPattern: Pattern.Pattern<unit>) : Pattern.Pattern<unit> =
    ...
    let patLiteral (literal: Literal.Literal) : Pattern.Pattern<unit> =
    ...
    /// </summary>
    let patUnit () : Pattern.Pattern<unit> = Pattern.UnitPattern()
