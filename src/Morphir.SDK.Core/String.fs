...
let slice (startIndex:int) (endIndex: int) (str:string) =
    let start = if startIndex >= 0 then startIndex else str.Length + startIndex
    let last = if endIndex >= 0 then endIndex-1 else (str.Length + endIndex - 1)
    str.[start..last]
...
let rec indexesHelp (substring:string) (str:string) (curr:int) (result: int list) =
    ...
    else indexesHelp substring (slice (idx+1) str.Length str) (curr+idx+1) (result @ [curr+idx])

let indexes (substring:string) (str:string) =
    indexesHelp substring str 0 []
...
let pad (n:int) (ch:char) (str:string) =
  ...
  repeat (ceil half |> int) (fromChar ch) + str + repeat (floor half |> int) (fromChar ch)
...
let trimLeft (str:string) =
    str.TrimStart()

let trimRight (str:string) =
    str.TrimEnd()

let map func str =
    String.map func str

let filter isGood str =
    String.filter isGood str
