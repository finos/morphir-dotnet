...
let inline ``member`` (key:'Key) (dict:Dict<'Key,'Value>) =
    Map.containsKey key dict

let inline size (dict:Dict<'Key,'Value>) = Map.count dict

let inline isEmpty (dict:Dict<'Key,'Value>) =
    Map.isEmpty dict
...
let inline toList (dict:Dict<'Key,'Value>) : List<'Key * 'Value> =
    Map.toList dict

let inline fromList assocs :Dict<'Key,'Value> =
    Map.ofList assocs
