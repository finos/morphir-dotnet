...
type LibraryBuilder() =
    ...
    member inline _.PackageName((), packageName: PackageName) = [
        LibraryProperty.PackageName packageName
    ]

    [<CustomOperation("packageName")>]
    member inline _.PackageName((), packageName: string) = [
        LibraryProperty.PackageName
        <| PackageName.fromString packageName
    ]
    ...
    member inline x.Run(props: LibraryProperty list) =
        ...
        |> List.fold
            (fun dist prop ->
                match prop with
                | LibraryProperty.PackageName packageName ->
                    Distribution.changePackageName packageName dist
                | LibraryProperty.Dependencies dependencies ->
                    Distribution.updateDependencies dependencies dist
                | LibraryProperty.Definition definition ->
                    Distribution.updateDefinition definition dist
                | LibraryProperty.Dependency(packageName, spec) ->
                    Distribution.insertDependency packageName spec dist
            )
            (emptyLibrary Package.PackageName.root)

let library = LibraryBuilder()

type ModuleProperty = ModuleName of moduleName: ModuleName
...
type ModuleBuilder() =
    ...
    [<CustomOperation("moduleName")>]
    member _.ModuleName((), moduleName: string) = [
...
module Demo =
    let result = library { packageName "MyPackage" }
