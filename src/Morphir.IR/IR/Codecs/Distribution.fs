...
let encoder (distro: Distribution) : Value =
    ...
    | Library(packagePath, dependencies, def) ->
        Encode.list id [
            ...
            def
            |> Package.encodeDefinition encodeUnit (Type.encoder encodeUnit)
        ]
...
let decodeVersionedDistribution: Decode.Decoder<Distribution> =
    Decode.oneOf [
        ...
        Decode.fail
            "The IR is in an old format that doesn't have a format version on it. Please regenerate the it!"
    ]
