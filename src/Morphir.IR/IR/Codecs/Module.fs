    ...
    Encode.object [
        ...
        |> Encode.list (fun (name, typeSpec) ->
            Encode.list id [
                Name.encoder name
                typeSpec
                |> Documented.encoder (Type.encodeSpecification encodeTypeAttributes)
            ]
        )
        "values",
        spec.Values
        ...
        |> Encode.list (fun (name, valueSpec) ->
            Encode.list id [
                Name.encoder name
                valueSpec
                |> Documented.encoder (Value.encodeSpecification encodeTypeAttributes)
            ]
        )
        "doc",
        spec.Doc
        |> Maybe.map Encode.string
        |> Maybe.withDefault Encode.nil
    ]
            ...
            (Decode.map
                ...
                (Decode.list (
                    Decode.map2
                        ...
                        (Decode.field
                            "spec"
                            (Documented.decoder (Type.decodeSpecification decodeTypeAttributes)))
                ))))
        (Decode.field
            "values"
            (Decode.map
                ...
                (Decode.list (
                    Decode.map2
                        ...
                        (Decode.index
                            1
                            (Documented.decoder (Value.decodeSpecification decodeTypeAttributes)))
                ))))
        (Decode.oneOf [
    ...
    Encode.object [
        ...
        |> Encode.list (fun (name, typeDef) ->
            Encode.list id [
                ...
                |> (AccessControlled.encoder (
                    Documented.encoder (Type.encodeDefinition encodeTypeAttributes)
                ))
            ]
        )
        "values",
        ...
        |> Encode.list (fun (name, valueDef) ->
            Encode.list id [
                ...
                |> (AccessControlled.encoder (
                    Documented.encoder (
                        Value.encodeDefinition encodeTypeAttributes encodeValueAttributes
                    )
                ))
            ]
        )
                    ...
                    Decode.map2
                        ...
                        (Decode.field
                            ...
                            (AccessControlled.decoder (
                                Documented.decoder (Type.decodeDefinition decodeTypeAttributes)
                            )))
                ))))
        (Decode.field
            "values"
                            ...
                            (AccessControlled.decoder (
                                Documented.decoder (
                                    Value.decodeDefinition
                                        decodeTypeAttributes
                                        decodeValueAttributes
                                )
                            )))
                ))))
        (Decode.oneOf [
