...
{
    /// <summary>
    /// Run unit tests (Morphir.Core.Tests, Morphir.Tooling.Tests, Morphir.Internal.CodeGeneration.Tests, Morphir.Models.Tests)
    /// Tests are run using TUnit and Expecto test frameworks
    /// </summary>
    Target Test => _ => _
        .DependsOn(Compile)
        .Description("Run unit tests (Morphir.Core.Tests, Morphir.Tooling.Tests, Morphir.Internal.CodeGeneration.Tests, Morphir.Models.Tests)")
        .Executes(() =>
        {
    ...
    void RunTests(string configuration)
    {
        ...
        if (coreExitCode != 0 || toolingExitCode != 0 || codeGenExitCode != 0 || modelsExitCode != 0)
        {
            Serilog.Log.Error("");
            Serilog.Log.Error("================================================");
